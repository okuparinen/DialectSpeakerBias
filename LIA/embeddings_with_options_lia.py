...
train = train.remove_columns(["Unnamed: 0", "orig_index", "SpeakerID", "duration", "original", "path", "DocID", "detailed", "simple", "split", "old_muni", "new_muni", "numeric_dialect", "named_dialect"])
...
dev = dev.remove_columns(["Unnamed: 0", "orig_index", "SpeakerID", "duration", "original", "path", "DocID", "detailed", "simple", "split", "old_muni", "new_muni", "numeric_dialect", "named_dialect"])
...
test = test.remove_columns(["Unnamed: 0", "orig_index", "SpeakerID", "duration", "original", "path", "DocID", "detailed", "simple", "split", "old_muni", "new_muni", "numeric_dialect", "named_dialect"])
...
if USE_VOICE_CONVERSION:
    ...
    matching_sets = [
        ...
        ("vc5", ref_generator('nos5')),
        ("vc6", ref_generator('nos6')),
        ("vc7", ref_generator('nos7')),
        ("vc8", ref_generator('nos8')),
        ("vc9", ref_generator('nos9'))
    ]
else:
    matching_sets = None
...
def extract_embeddings(
    ...
    """
    Extract embeddings for original audio and multiple voice conversion sets.
    - Memory-optimized: writes incrementally to CSV.
    - tqdm progress bars: batches + total variants×samples remaining.
    - Resume support: derives progress from existing CSV row counts.
    """
    ...
    # Create directories and initialize file paths
    file_paths = {}
    ...
    def count_csv_rows(path: str) -> int:
        ...
        if not os.path.exists(path):
            return 0
        # One header written initially, then appends without header.
        # So rows = total lines - 1 (header).
        with open(path, 'r', encoding='utf-8', errors='ignore') as f:
            # Using sum over iterator to avoid loading file in memory
    ...
    def process_variant(batch_arrays, suffix, batch_indices):
        """
        Writes only the portion of the batch that hasn't been processed yet per layer.
        Also updates per-layer and per-variant progress counters.
        """
        ...
        for L in layers_to_export:
            state = o.last_hidden_state if L == 24 else o.hidden_states[L + offset]
            means = torch.mean(state, dim=1)
            stds = torch.std(state, dim=1)

            # Determine how many items in this batch we need to skip for this layer
            layer_done_upto = processed_counts[suffix][L]  # number of samples already written
            keep_mask = [idx >= layer_done_upto for idx in batch_indices]

            # If nothing to write for this layer, continue
            ...
            # Accumulate only rows we need to append
            ...
            # Update per-layer processed count by number of rows we just wrote
            written_n = len(df_full)
            processed_counts[suffix][L] += written_n

        # After all layers, update variant-level progress and progress bar by delta
    ...
    # Advance batch progress bar to reflect already traversed batches
